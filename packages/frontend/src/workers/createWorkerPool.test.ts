...
describe('createWorkerPool', () => {
  ...
  describe('fallback 参数传递', () => {
    it('fallback 应接收正确的 type 和 payload', async () => {
      const workerBackup = globalThis.Worker;
      // @ts-ignore
      delete globalThis.Worker;
      ...
      await pool.execute('myType', payload);

      expect(fallback).toHaveBeenCalledWith('myType', payload);
      // @ts-ignore
      globalThis.Worker = workerBackup;
      pool.destroy();
    });

    it('fallback 可以返回 Promise（异步降级）', async () => {
      const workerBackup = globalThis.Worker;
      // @ts-ignore
      delete globalThis.Worker;
  ...
  describe('execute 后立即 destroy', () => {
    it('pending 任务在 destroy 时应全部被拒绝', async () => {
      const createWorkerPool = await getCreateWorkerPool();
      const pool = createWorkerPool(workerUrl, { size: 2 });
      ...
      await expect(p1).rejects.toThrow('Worker pool 已销毁');
