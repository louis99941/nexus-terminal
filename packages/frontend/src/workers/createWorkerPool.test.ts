...
describe('createWorkerPool', () => {
  ...
  describe('所有 Worker 忙碌时的队列行为', () => {
    it('所有 Worker 忙碌时第三个任务应等待后通过 setInterval 轮询获取空闲 Worker', async () => {
      ...
      // Resolve task2
      createdWorkers[0].simulateMessage({ id: call2.id, type: 'task2', payload: 'result2' });
      const r2 = await p2;
    ...
    it('hasIdle 应在所有 Worker 忙碌时返回 false', async () => {
      const createWorkerPool = await getCreateWorkerPool();
      const pool = createWorkerPool(workerUrl, { size: 1 });
    ...
    it('两个 Worker 时第三个任务应在任一 Worker 完成后执行', async () => {
      ...
      const call1 = createdWorkers[0].postMessage.mock.calls[0][0];
      createdWorkers[0].simulateMessage({ id: call1.id, type: 'task1', payload: 'r1' });
      await p1;
      ...
      createdWorkers[task3WorkerIdx].simulateMessage({ id: call3.id, type: 'task3', payload: 'r3' });
      const r3 = await p3;
      expect(r3).toBe('r3');

      // Resolve task2 as well
      const call2 = createdWorkers[1].postMessage.mock.calls[0][0];
      createdWorkers[1].simulateMessage({ id: call2.id, type: 'task2', payload: 'r2' });
      const r2 = await p2;
      expect(r2).toBe('r2');
