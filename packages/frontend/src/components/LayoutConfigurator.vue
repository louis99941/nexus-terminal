...
function removePaneFromAvailableList(paneName: PaneName) {
  if (paneName === 'terminal') {
    ...
    if (index > -1) {
      ...
      log.info(`[LayoutConfigurator] Removed 'terminal' from available panes.`);
    }
  }
}
// --- Computed ---
...
const saveLayout = async () => {
  // Make async
  log.info('[LayoutConfigurator] Attempting to save layout...');
  try {
    // Save main layout and wait for persistence
    log.info('[LayoutConfigurator] Updating main layout tree in store...');
    ...
    log.info('[LayoutConfigurator] Layout saved successfully, dialog closed.');
  } catch (error: unknown) {
    log.error('[LayoutConfigurator] Error saving layout:', error);
  }
};
...
const clonePane = (paneName: PaneName): LayoutNode => {
  log.info(`[LayoutConfigurator] Cloning pane: ${paneName}`);
  return {
...
const handleNodeUpdate = (updatedNode: LayoutNode) => {
  log.info('[LayoutConfigurator] Received node update from editor:', updatedNode);
...
const handleNodeRemove = async (payload: {
  ...
  log.info('[LayoutConfigurator] Received node remove request:', payload);
...
const onDraggableChange = (event: DraggableChangeEvent, side: 'left' | 'right') => {
  // Add side parameter
  log.info(`[LayoutConfigurator] Draggable change event detected on ${side} sidebar:`, event);
  ...
  if (event.added) {
    ...
    log.info(`[LayoutConfigurator] Item moved or removed within/from ${side} sidebar.`);
  }
...
const handleAvailablePaneDragEnd = (event: DraggableEndEvent) => {
  ...
  if (event.to !== event.from) {
    ...
    if (paneName === 'terminal') {
      ...
      log.info(`[LayoutConfigurator] Non-terminal pane "${paneName}" dropped elsewhere (clone).`);
    ...
    log.info('[LayoutConfigurator] Item dropped back into available list or drag cancelled.');
  }
};
</script>
