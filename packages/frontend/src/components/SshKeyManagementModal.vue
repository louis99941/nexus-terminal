...
const showEditForm = async (key: SshKeyBasicInfo) => {
  ...
  if (details) {
    ...
    uiNotificationsStore.addNotification({
      message: t('sshKeys.modal.errorFetchDetails'),
      type: 'error',
    });
  ...
  <div class="fixed inset-0 bg-overlay flex justify-center items-center z-50 p-4">
    ...
    >
      ...
      <div v-if="!isAddEditFormVisible" class="flex flex-col h-full">
        ...
        <!-- Key List -->
        <div
          class="flex-grow overflow-y-auto border border-border rounded-md"
          ...
          <table class="min-w-full divide-y divide-border">
            ...
            <tbody class="bg-background divide-y divide-border">
              <tr v-if="isLoading">
                <td
                  colspan="2"
                  class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary text-center"
                >
