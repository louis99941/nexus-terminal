...
const getInitialLocale = (): string => {
  ...
  if (storedLocale && availableLocales.includes(storedLocale)) {
    log.info(`[i18n] Using locale from localStorage: ${storedLocale}`);
    return storedLocale;
  }
  ...
  if (navigatorLocale && availableLocales.includes(navigatorLocale)) {
    log.info(`[i18n] Using locale from navigator.language: ${navigatorLocale}`);
  ...
  if (navigatorLangPart && availableLocales.includes(navigatorLangPart)) {
    log.info(`[i18n] Using locale based on navigator language part: ${navigatorLangPart}`);
    return navigatorLangPart;
  }

  // 4. 最后回退到默认语言
  log.info(`[i18n] Falling back to default locale: ${defaultLng}`);
  return defaultLng;
};
...
export const setLocale = (lang: string) => {
  log.info(`[i18n] Attempting to set locale to: ${lang}`);
  ...
  if (availableLocales.includes(lang)) {
    ...
    if (currentLocale !== lang) {
      ...
      log.info(`[i18n] Successfully updated global locale from "${currentLocale}" to "${lang}".`);
      try {
        localStorage.setItem(localStorageKey, lang); // 持久化到 localStorage
        log.info(`[i18n] Locale "${lang}" saved to localStorage.`);
      } catch (error: unknown) {
        log.error('[i18n] Failed to save locale to localStorage:', error);
      }
    } else {
      log.info(`[i18n] Locale is already "${lang}". No update needed.`);
    }
  } else {
    log.warn(
