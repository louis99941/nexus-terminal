...
describe('useVirtualListSetup 回归', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('函数形式 itemHeight 返回 40px 时 overscan 应为 5（min clamp）', () => {
    const dataSource = ref([1]);
    useVirtualListSetup(dataSource, { itemHeight: () => 40 });
    // Math.ceil(200/40) = 5, clamp(5, 5, 15) = 5
    expect(useVirtualList).toHaveBeenCalledWith(
  ...
  it('函数形式 itemHeight 返回 15px 时 overscan 应为 14', () => {
    const dataSource = ref([1]);
    useVirtualListSetup(dataSource, { itemHeight: () => 15 });
    // Math.ceil(200/15) = 14, clamp(14, 5, 15) = 14
    expect(useVirtualList).toHaveBeenCalledWith(
  ...
  it('overscan=0（显式）不应触发自动计算', () => {
    const dataSource = ref([1]);
    // Even though itemHeight=50 would auto-compute to 5, explicit overscan=0 wins
    useVirtualListSetup(dataSource, { itemHeight: 50, overscan: 0 });
    expect(useVirtualList).toHaveBeenCalledWith(
      dataSource,
      expect.objectContaining({ overscan: 0 })
  ...
  it('审计日志使用 itemHeight=180, overscan=10 的组合应能正常工作', () => {
    // This mirrors the AuditLogView.vue usage after the PR change
    const dataSource = ref([{ id: 1, action: 'login' }, { id: 2, action: 'logout' }]);
    useVirtualListSetup(dataSource, { itemHeight: 180, overscan: 10 });
    expect(useVirtualList).toHaveBeenCalledWith(
      dataSource,
      expect.objectContaining({ itemHeight: 180, overscan: 10 })
    );
  });

  it('连接列表使用 itemHeight=数值, overscan=10 的组合应能正常工作', () => {
    // This mirrors the WorkspaceConnectionList.vue usage after the PR change
    const CONNECTION_ITEM_HEIGHT = 48; // typical connection item height
    const dataSource = ref([{ id: '1' }, { id: '2' }]);
    useVirtualListSetup(dataSource, { itemHeight: CONNECTION_ITEM_HEIGHT, overscan: 10 });
    expect(useVirtualList).toHaveBeenCalledWith(
      dataSource,
      expect.objectContaining({ itemHeight: CONNECTION_ITEM_HEIGHT, overscan: 10 })
    );
  });

  it('文件列表使用 overscan=15 应能正常工作', () => {
    // This mirrors the FileManagerFileList.vue usage after the PR change
    const heightFn = () => 36;
    const dataSource = ref([{ name: 'file1.txt' }, { name: 'file2.txt' }]);
    useVirtualListSetup(dataSource, { itemHeight: heightFn, overscan: 15 });
    expect(useVirtualList).toHaveBeenCalledWith(
      dataSource,
      expect.objectContaining({ itemHeight: heightFn, overscan: 15 })
    );
  });
});