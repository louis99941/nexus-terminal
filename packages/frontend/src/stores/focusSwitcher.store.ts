...
export const useFocusSwitcherStore = defineStore('focusSwitcher', () => {
  ...
  async function loadConfigurationFromBackend() {
    ...
    try {
      ...
      if (typeof loadedFullConfig?.shortcuts === 'object' && loadedFullConfig.shortcuts !== null) {
        ...
        for (const id of Object.keys(loadedFullConfig.shortcuts)) {
          if (availableIds.has(id)) {
            ...
            log.warn(`[FocusSwitcherStore] Ignoring shortcut config for unknown ID: ${id}`);
          }
        }
      ...
      sequenceOrder.value = [];
      itemConfigs.value = {};
      // log.info('[FocusSwitcherStore] Reset sequenceOrder and itemConfigs due to loading error.');
    }
  }
  ...
  function triggerTerminalSearchActivation() {
    ...
    // log.info('[FocusSwitcherStore] Triggering Terminal search activation.');
  }

  function triggerFileManagerSearchActivation() {
    activateFileManagerSearchTrigger.value++;
    // log.info('[FocusSwitcherStore] Triggering FileManager search activation.');
  }
  ...
  function toggleConfigurator(visible?: boolean) {
    ...
    // log.info(`[FocusSwitcherStore] Configurator visibility set to: ${isConfiguratorVisible.value}`);
  }
  ...
  */
  ...
  function updateConfiguration(newFullConfig: FocusSwitcherFullConfig) {
    // log.info('[FocusSwitcherStore] updateConfiguration called with new full configuration:', JSON.stringify(newFullConfig));
    const availableIds = new Set(availableInputs.value.map((input) => input.id));
  ...
  function registerFocusAction(
    ...
    // log.info(`[FocusSwitcherStore] Registered focus action for ID: ${id}. Total actions for this ID: ${actions.length}`);
    ...
    const unregister = () => {
      ...
      if (currentActions) {
        ...
        if (index > -1) {
          ...
          // log.info(`[FocusSwitcherStore] Unregistered a focus action for ID: ${id}. Remaining actions: ${currentActions.length}`);
          // 如果数组为空，可以从 Map 中移除该 ID
          if (currentActions.length === 0) {
            registeredActions.value.delete(id);
            // log.info(`[FocusSwitcherStore] Removed ID ${id} from registeredActions map as it has no more actions.`);
          }
        } else {
          log.warn(
  ...
  //    log.warn("[FocusSwitcherStore] unregisterFocusAction(id) is deprecated. Use the function returned by registerFocusAction instead.");
  // }

  // 修改：统一的聚焦目标 Action，现在迭代 Map 中的动作数组
  async function focusTarget(id: string): Promise<boolean> {
    // log.info(`[FocusSwitcherStore] Attempting to focus target ID: ${id}`);
    const actions = registeredActions.value.get(id);

    if (!actions || actions.length === 0) {
      log.warn(`[FocusSwitcherStore] No focus actions registered for ID: ${id}`);
      return false;
    }

    // log.info(`[FocusSwitcherStore] Found ${actions.length} action(s) for ID: ${id}. Iterating...`);
    ...
    for (const action of actions) {
      try {
        ...
        if (result === true) {
          ...
          // log.info(`[FocusSwitcherStore] Successfully focused ${id} via one of its actions.`);
          return true;
        }
        if (result === false) {
          // 如果动作返回 false，表示尝试但失败，记录日志并继续下一个动作
          // log.info(`[FocusSwitcherStore] An action for ${id} returned false (failed). Trying next action if available.`);
        } else if (result === undefined) {
          // 如果动作返回 undefined，表示跳过（例如非活动实例），记录日志并继续下一个动作
          // log.info(`[FocusSwitcherStore] An action for ${id} returned undefined (skipped). Trying next action if available.`);
        }
        // 如果 result 是其他值，也视为跳过或未处理
      } catch (error: unknown) {
        log.error(`[FocusSwitcherStore] Error executing a focus action for ${id}:`, error);
        // 即使出错，也继续尝试下一个动作
      }
    }

    // 如果遍历完所有动作都没有成功聚焦 (没有返回 true)
    // log.info(`[FocusSwitcherStore] All actions for ${id} executed, but none returned true. Focus failed.`);
