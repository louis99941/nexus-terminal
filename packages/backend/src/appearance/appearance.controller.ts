...
export const getAppearanceSettingsController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 获取外观设置失败:', error);
    next(error);
  }
};
...
export const updateAppearanceSettingsController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 更新外观设置失败:', error);
    next(error);
  }
};
...
export const getBackgroundFileController = async (
  ...
  try {
    ...
    res.sendFile(absolutePath, (err) => {
      if (err) {
        logger.error(`[AppearanceController] 发送文件时出错 (${absolutePath}):`, err);
        // 避免在已发送响应头后再次发送
        if (!res.headersSent) {
          next(err);
        }
    ...
    if (getErrorCode(error) === 'ENOENT') {
      logger.warn(`[AppearanceController] 请求的背景文件未找到: ${filename}`);
      res.status(404).json({ message: '文件未找到' });
    } else {
      logger.error(`[AppearanceController] 获取背景文件时出错 (${filename}):`, error);
      next(error);
    }
  }
};
...
export const removePageBackgroundController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 移除页面背景失败:', error);
    next(error);
  }
};
...
export const removeTerminalBackgroundController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 移除终端背景失败:', error);
    next(error);
  }
};
...
export const listLocalHtmlPresetsController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 获取 HTML 主题列表失败:', error);
    next(error);
  }
};
...
export const getLocalHtmlPresetContentController = async (
  ...
  try {
    ...
    logger.error(`[AppearanceController] 获取主题 '${req.params.themeName}' 内容失败:`, error);
    next(error);
  }
};
...
export const createLocalHtmlPresetController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 创建用户自定义 HTML 主题失败:', error);
    next(error);
  }
};
...
export const updateLocalHtmlPresetController = async (
  ...
  try {
    ...
    if (getErrorMessage(error).includes('未找到')) {
      ...
      logger.error('[AppearanceController] 更新用户自定义 HTML 主题失败:', error);
      next(error);
    }
  }
};
...
export const deleteLocalHtmlPresetController = async (
  ...
  try {
    ...
    if (getErrorMessage(error).includes('未找到')) {
      ...
      logger.error('[AppearanceController] 删除用户自定义 HTML 主题失败:', error);
      next(error);
    }
  }
};
...
export const getRemoteHtmlPresetsRepositoryUrlController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 获取远程 HTML 主题仓库链接失败:', error);
    next(error);
  }
};
...
export const updateRemoteHtmlPresetsRepositoryUrlController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 更新远程 HTML 主题仓库链接失败:', error);
    next(error);
  }
};
...
export const listRemoteHtmlPresetsController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 获取远程 HTML 主题列表失败:', error);
    next(error);
  }
};
...
export const getRemoteHtmlPresetContentController = async (
  ...
  try {
    ...
    logger.error('[AppearanceController] 获取远程 HTML 主题内容失败:', error);
    next(error);
  }
};
