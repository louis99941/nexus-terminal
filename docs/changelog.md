...
- 🗜️ 修复解压大压缩包长时间无响应最终失败：解压命令的全部输出写入 stdout 而未被消费，SSH 通道接收窗口耗尽后远程进程阻塞挂起（#112）
- ⏱️ 修复服务器缺少 unzip/zip/tar 命令时，压缩/解压操作迟迟无反馈直至 120 秒超时；现在立即提示命令未找到，并修正该类错误消息的会话路由（#112）
- 📁 修复多连接窗口下 SFTP 通道断开后目录打包下载失败（404）：下载时按需自动重建缺失的 SFTP 会话（#113）

### 测试

- 🧪 新增真实 SSH/SFTP 协议集成测试桩，覆盖解压通道流控、命令缺失提示、多窗口与并发目录打包下载、SFTP 会话自动重建等场景

## v1.5.7（2026-07-10）

### 改进

- 🔍 SFTP 下载支持 sessionId 精确匹配，避免多会话场景下匹配到错误的 SFTP 实例
- 📁 路径安全字符扩展，支持包含冒号的时间戳目录路径（如 `2026-07-08T05:49:42Z`）
- 🐳 新增本地 Docker Compose 测试环境配置

### 修复

- 📊 修复 CPU 监控 iowait 未计入空闲时间导致使用率被高估的问题

## v1.5.6（2026-07-02）

### 改进

- 📊 SFTP 文件上传进度条平滑显示，不再从 0% 直接跳到完成
- 🗜️ 压缩/解压操作新增实时进度弹窗，显示已处理文件数和当前文件名

### 修复

- 🌐 修正日语（ja-JP）语言文件中压缩进度的中文文本
